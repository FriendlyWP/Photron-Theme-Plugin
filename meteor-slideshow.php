	...
	<?php endif; ?>
		...
		while ( $meteor_loop->have_posts() ) : $meteor_loop->the_post(); ?>
			...
			if ( $meteor_count == 1 && $meteor_loop->post_count > 1 ) {
				...
				echo '<img style="visibility: hidden;" class="meteor-shim" src="' . $meteor_shim[0] . '" alt="' . $alt . '" />';
			...
			<div class="mslide mslide-<?php echo $meteor_count; ?>">
				...
				<?php 
				$headline = get_the_title($meteor_loop->ID);
						if (function_exists('get_field')) {
							/*if ( get_field('headline', $post->ID)) {
								$headline = get_field('headline', $post->ID);
							}*/

							if ( get_field('copy', $meteor_loop->ID)) {
								$copy = get_field('copy', $meteor_loop->ID);
							}

							if ( get_field('button_text', $post->ID)) {
								$button_text = get_field('button_text', $post->ID);
							}

							if ($headline || $copy || $button_text) {

								if ( get_post_meta( $post->ID, "slide_url_value", $single = true ) != "" ) { ?>
									<a href="<?php echo get_post_meta( $post->ID, "slide_url_value", $single = true ); ?>">
								<?php } 
								...
								if ($button_text !== '') {
									echo '<button class="slide-button">' . $button_text . '</button>';
								}
							echo '</span>';
							echo '</div>';
							if ( get_post_meta( $post->ID, "slide_url_value", $single = true ) != "" ) { 
				...
				if ( get_post_meta( $post->ID, "slide_url_value", $single = true ) != "" ) { ?>
					...
					<a href="<?php echo get_post_meta( $post->ID, "slide_url_value", $single = true ); ?>"><?php the_post_thumbnail( 'featured-slide', array( 'alt' => the_title_attribute('echo=0') ) ); ?></a>
